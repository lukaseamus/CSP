...
np.alt <- ggplot() +
        ...
        geom_ribbon(data = new, aes(age, ymin = npp.lo*1000, ymax = npp.hi*1000, fill = sp),
                    alpha = 0.5) +
        geom_point(aes(age, npp*1000, colour = sp), 
                   alpha = 0.4, shape = 16, size = 3, position = position_dodge(width = 1.5)) +
        geom_pointrange(data = npp.stat, aes(group2, mean*1000, ymin = mean*1000 - se*1000*qnorm(0.975),
                                             ymax = mean*1000 + se*1000*qnorm(0.975), colour = group1),
                        size = 0.5, position = position_dodge(width = 1.5)) +
        scale_colour_manual(values = c("#333b08","#627d0e","#f1c700"),
                            labels = c(expression(italic("L. digitata")*"        y = –0.001x + 1"),
                                       expression(italic("L. hyperborea")*"  y = –0.009x + 0.97"),
        ...
        xlab("Detrital age (d)") +
        scale_x_continuous(breaks = seq(0, 35, by = 5)) +
        scale_y_continuous(breaks = seq(-1, 2, by = 0.5), expand = c(0, 0)) +
        coord_cartesian(ylim = c(-1, 2), xlim = c(0.5, 33.36)) +
        theme(legend.position = c(0.4, 0.905)) +
        mytheme

np.alt # dimsenisons: 4 x 5 in
...
gp.alt <- ggplot() +
        ...
        geom_ribbon(data = new, aes(age, ymin = gpp.lo*1000, ymax = gpp.hi*1000, fill = sp),
                    alpha = 0.5) +
        geom_point(aes(age, gpp*1000, colour = sp), 
                   alpha = 0.4, shape = 16, size = 3, position = position_dodge(width = 1.5)) +
        geom_pointrange(data = gpp.stat, aes(group2, mean*1000, ymin = mean*1000 - se*1000*qnorm(0.975),
                                             ymax = mean*1000 + se*1000*qnorm(0.975), colour = group1),
                        size = 0.5, position = position_dodge(width = 1.5)) +
        scale_colour_manual(values = c("#333b08","#627d0e","#f1c700"),
                            labels = c(expression(italic("L. digitata")*"        y = 0.007x + 1.27"),
        ...
        scale_x_continuous(breaks = seq(0, 35, by = 5)) +
        scale_y_continuous(expand = c(0, 0)) +
        coord_cartesian(ylim = c(-1, 3), xlim = c(0, 33.34)) +
        theme(legend.position = c(0.66, 0.905)) +
        mytheme

gp.alt # dimsenisons: 4 x 5 in

#### 5.5  Combined ####
require(cowplot)
# plot pp.alt needs to be loaded from the Decomposition.R file
Fig.5 <- plot_grid(np.alt, pp.alt, rel_widths = c(1.4, 1), label_fontfamily = "Helvetica Neue",
                   label_size = 15, labels = "auto", hjust = 0.5)
Fig.5 # dimensions 4 x 8.5 in
