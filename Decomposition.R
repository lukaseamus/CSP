...
m7 <- lmer(loss ~ phen * sp + (age|bag), REML = F) # mixed effects model
...
m11 <- lmer(loss ~ phen + (1|bag), REML = F) # mixed effects model
...
m27 <- lmer(loss ~ C * sp + (age|bag), REML = F) # mixed effects model
# singular fit -> simplify mixed effects model
...
# homogeneity seems slightly improved

#### 6.7  Interpret model ####
Anova(m28, type = 2) # Type II Sums of Squares test
# Response: loss
#   Df  Chisq Pr(>Chisq)    
# C  1 14.524  0.0001384 ***

coef(m28) # y = -0.1728658x + 5.8251400


#### 7.   Grazing data analysis ####
#### 7.1  Determine random components ####
m30 <- lm(ex ~ sp2) # fixed effects model
m31 <- lmer(ex ~ sp2 + (1|bag2), REML = F) # mixed effects model
# singular fit -> model is too complex
anova(m31, m30) # models are not different
# continue with m30
...
#### 7.3  Determine random components ####
m32 <- glm(ex+1 ~ sp2, family = Gamma(link = "log"))
m33 <- glmer(ex+1 ~ sp2 + (1|bag2), family = Gamma(link = "log"))
anova(m33, m32) # models are not different
# continue with m32
...
#### 7.6  Determine random components ####
m34 <- lm(per ~ sp2) # fixed effects model
m35 <- lmer(per ~ sp2 + (1|bag2), REML = F) # mixed effects model
# singular fit -> model is too complex
...
#### 7.8  Determine random components ####
m36 <- glm(per+1 ~ sp2, family = Gamma(link = "log"))
m37 <- glmer(per+1 ~ sp2 + (1|bag2), family = Gamma(link = "log"))
...
C.stat <- describeBy(C, list(sp, age), mat = T)
C.stat$group2 <- as.integer(C.stat$group2)
C.stat2 <- describeBy(C, sp, mat = T)

CN.stat <- describeBy(CN, list(sp, age), mat = T)
CN.stat$group2 <- as.integer(CN.stat$group2)

Cl.stat <- cbind(C.stat2[,c(2,5,15)], loss.stat[,c(5, 15)])
rownames(Cl.stat) <- NULL
colnames(Cl.stat) <- c("species","C","Cse","D","Dse")
...
m17 <- lme(C ~ sp * age,
           random = ~age|bag,
           weights = varExp(form = ~age|sp))
new$C.fit <- predict(m17, level = 0, newdata = new)
modmat <-  model.matrix(formula(m17)[-2], new)
int <- diag(modmat %*% vcov(m17) %*% t(modmat))
new$C.lo <- with(new, C.fit - qnorm(0.975)*sqrt(int))
new$C.hi <- with(new, C.fit + qnorm(0.975)*sqrt(int))

# Carbon-nitrogen ratio
m24 <- glm(CN ~ sp + age, family = Gamma(link = "log"))
inv <- family(m24)$linkinv # calculate inverse of link function
predicted <- predict(m24, type = "link", se.fit = TRUE, newdata = new)
new$CN.fit <- inv(predicted$fit)
new$CN.lo <- inv(predicted$fit - predicted$se.fit * qnorm(0.975))
new$CN.hi <- inv(predicted$fit + predicted$se.fit * qnorm(0.975))
...
m28 <- gls(loss ~ C, 
           weights = varExp(form = ~C))
Biochem$C.fit <- predict(m28)
modmat <-  model.matrix(formula(m28)[-2])
int <- diag(modmat %*% vcov(m28) %*% t(modmat))
Biochem$C.lo <- with(Biochem, C.fit - qnorm(0.975)*sqrt(int))
Biochem$C.hi <- with(Biochem, C.fit + qnorm(0.975)*sqrt(int))

#### 8.3  Customised theme ####
require(ggplot2)
...
Cp <- ggplot()+
        geom_line(data = new, aes(age, C.fit, colour = sp, lty = sp), size = 0.5) +
        geom_ribbon(data = new, aes(age, ymin = C.lo, ymax = C.hi, fill = sp),
                    alpha = .5) +
        geom_pointrange(data = C.stat, aes(group2, mean, ymin = mean - se,
                                           ymax = mean + se, colour = group1),
                        size = 0.5, position = position_dodge(width = 2.16)) +
        scale_colour_manual(values = c("#333b08","#627d0e","#f1c700"),
                            labels = c(expression(italic("L. digitata")*"        y = 0.14x + 24.77"),
                                       expression(italic("L. hyperborea")*"  y = 0.01x + 28.46"),
                                       expression(italic("L. ochroleuca")*"  y = –0.19x + 26.4")),
                            guide = guide_legend()) +
        scale_fill_manual(values = c("#333b08","#627d0e","#f1c700"),
                          labels = c(expression(italic("L. digitata")*"        y = 0.14x + 24.77"),
                                     expression(italic("L. hyperborea")*"  y = 0.01x + 28.46"),
        ...
        scale_y_continuous(expand = c(0,0)) +
        scale_x_continuous(expand = c(0,0)) +
        coord_cartesian(ylim = c(15, 40), xlim = c(10, 35)) +
        theme(legend.position = c(0.62, 0.905)) +
        mytheme

Cp

CNp <- ggplot()+
        geom_line(data = new, aes(age, CN.fit, colour = sp), size = 0.5) +
        geom_ribbon(data = new, aes(age, ymin = CN.lo, ymax = CN.hi, fill = sp),
                    alpha = .5) +
        geom_pointrange(data = CN.stat, aes(group2, mean, ymin = mean - se,
                                           ymax = mean + se, colour = group1),
                        size = 0.5, position = position_dodge(width = 2.16)) +
        scale_colour_manual(values = c("#333b08","#627d0e","#f1c700"),
                            labels = c(expression(italic("L. digitata")*"        y = "*e^"–0.005x + 2.93"),
                                       expression(italic("L. hyperborea")*"  y = "*e^"–0.005x + 2.94"),
                                       expression(italic("L. ochroleuca")*"  y = "*e^"–0.005x + 2.97")),
                            guide = guide_legend()) +
        scale_fill_manual(values = c("#333b08","#627d0e","#f1c700"),
                          labels = c(expression(italic("L. digitata")*"        y = "*e^"–0.005x + 2.93"),
                                     expression(italic("L. hyperborea")*"  y = "*e^"–0.005x + 2.94"),
        ...
        scale_y_continuous(expand = c(0,0)) +
        scale_x_continuous(expand = c(0,0)) +
        coord_cartesian(ylim = c(14, 22), xlim = c(10, 35)) +
        theme(legend.position = c(0.58, 0.905)) +
        mytheme
...
Cp2 <- ggplot() +
        geom_line(data = Biochem, aes(C, C.fit)) +
        geom_ribbon(data = Biochem, aes(C, ymin = C.lo, ymax = C.hi), 
                    fill = NA, colour = "#000000", alpha = .5) +
        geom_pointrange(data = Cl.stat, 
                        aes(C, D, ymin = D - Dse, ymax = D + Dse, colour = species)) +
        geom_errorbarh(data = Cl.stat, 
                       aes(y = D, xmin = C - Cse, xmax = C + Cse, colour = species), 
                       height = 0) +
        geom_text(data = Biochem, aes(20.25, 0.6), label = "y = –0.17x + 5.83", 
        ...
        xlab("Carbon content (%)") +
        scale_y_continuous(expand = c(0, 0)) +
        scale_x_continuous(expand = c(0, 0), breaks = seq(20, 30, by = 2)) +
        coord_cartesian(ylim = c(0.5, 2.5), xlim = c(20, 30)) +
        theme(legend.position = c(0.8, 0.905)) +
        mytheme
Cp2
...
pp3 <- ggplot() +
        ...
        geom_pointrange(data = pl.stat, 
                        aes(P, D, ymin = D - Dse, ymax = D + Dse, colour = species)) +
        geom_errorbarh(data = pl.stat, 
                       aes(y = D, xmin = P - Pse, xmax = P + Pse, colour = species), 
                       height = 0) +
        geom_text(data = Biochem, aes(0.04, 0.6), label = "y = –1.05x + 1.66", 
                  family = "Helvetica Neue", check_overlap = T, hjust = 0, size = 4.2) +
        scale_colour_manual(values = c("#333b08","#627d0e","#f1c700"),
                            labels = c(expression(italic("L. digitata")),
                                       expression(italic("L. hyperborea")),
        ...
        scale_y_continuous(expand = c(0, 0)) +
        scale_x_continuous(expand = c(0, 0)) +
        coord_cartesian(ylim = c(0.5, 2.5), xlim = c(0, 1.5)) +
        theme(legend.position = c(0.2, 0.905)) +
        mytheme
pp3

#### 6.5  Combined plots ####
require(cowplot)
dp <- dp + theme(plot.margin = unit(c(.2, .5, .2, .5),"cm"))
plots <- align_plots(dp, exp, align = "h", axis = "t")
right_col <- plot_grid(plots[[2]], perp, labels = c("b", "c"), 
                       label_size = 15, nrow = 2)
DG <- plot_grid(plots[[1]], right_col, labels = c("a", ""),
                label_size = 15, rel_widths = c(1.048, 1))
DG # dimensions 4 x 8.5 in

pp3 <- pp3 + theme(axis.title.y = element_blank(),
                   axis.text.y = element_blank(),
                   plot.margin = unit(c(.2, .5, .2, .5),"cm"),
                   legend.position = "none")
CP <- plot_grid(Cp2, pp3, labels = "auto", label_size = 15,
                rel_widths = c(1.086, 1), hjust = 0.5)
CP # dimensions 4 x 8.5 in

CC <- plot_grid(Cp, CNp, labels = "auto", label_size = 15)
CC # dimensions 4 x 7 in
