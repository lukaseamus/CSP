...
WH <- cbind(-4.144978, 50.363629) # coordinates for West Hoe
temp19 <- extract(had.sst[[1789:1800]], WH, method = "bilinear") # extract data

mtemp <- mean(temp19) # annual mean = 13.43264°C 
sptemp <- mean(temp19[3:5]) # spring mean = 11.2797°C
sutemp <- mean(temp19[6:8]) # summer mean = 16.31189°C
autemp <- mean(temp19[9:11]) # autumn mean = 15.0291°C
witemp <- mean(temp19[c(12, 1:2)]) # winter mean = 11.10985°C

mtdiff <- mtemp - fieldtemp # the temperature differential between the measured temperature 
# and the annual average is -0.06603443°C
# this means that decomposition needs to be down-corrected because the annual
# mean temperature is slightly lower than the experimental temperature

# calculate temperature differentials for different seasons
sptdiff <- sptemp - fieldtemp 
sutdiff <- sutemp - fieldtemp 
autdiff <- autemp - fieldtemp 
witdiff <- witemp - fieldtemp
...
RH <- c(-4.221083, 50.31164) # (doi 10.1002/ecm.1366)
...
od <- function(time, temperature) {1 - time * (0.0153889922394093 + 0.00051 * temperature)} # L. ochroleuca decomposition (proportion left d-1)
hd <- function(time, temperature) {1 - time * (0.00603222583027008 + 0.00051 * temperature)} # L. hyperborea decomposition (proportion left d-1)
dd <- function(time, temperature) {1 - time * (0.00926997162947394 + 0.00051 * temperature)} # L. digitata decomposition (proportion left d-1)

# constrained decomposition (proportion left >= 0)
od0 <- function(time, temperature) {
  deco <- 1 - time * (0.0153889922394093 + 0.00051 * temperature)
  if(time < 66 && temperature == mtdiff){
    deco
  } else if(time < 71 && temperature == sptdiff){
    deco
  } else if(time < 60 && temperature == sutdiff){
    deco
  } else if(time < 62 && temperature == autdiff){
    deco
  } else if(time < 71 && temperature == witdiff){
    deco
  } else {
    0
  }
...
hd0 <- function(time, temperature) {
  deco <- 1 - time * (0.00603222583027008 + 0.00051 * temperature)
  if(time < 167 && temperature == mtdiff){
    deco
  } else if(time < 205 && temperature == sptdiff){
    deco
  } else if(time < 134 && temperature == sutdiff){
    deco
  } else if(time < 147 && temperature == autdiff){
    deco
  } else if(time < 208 && temperature == witdiff){
    deco
  } else {
    0
  }
...
dd0 <- function(time, temperature) {
  deco <- 1 - time * (0.00926997162947394 + 0.00051 * temperature)
  if(time < 109 && temperature == mtdiff){
    deco
  } else if(time < 123 && temperature == sptdiff){
    deco
  } else if(time < 94 && temperature == sutdiff){
    deco
  } else if(time < 100 && temperature == autdiff){
    deco
  } else if(time < 125 && temperature == witdiff){
    deco
  } else {
    0
  }
...
CSP <- function (density, species, period, time, temperature) {
  
  d <- density * c$carbon[1] * dd(time, temperature)
  h <- density * c$carbon[2] * hd(time, temperature)
  o <- density * c$carbon[3] * od(time, temperature)

  dsp <- density * c$carbon[4] * dd(time, temperature)
  hsp <- density * c$carbon[5] * hd(time, temperature)
  osp <- density * c$carbon[6] * od(time, temperature)
  
  dsu <- density * c$carbon[7] * dd(time, temperature)
  hsu <- density * c$carbon[8] * hd(time, temperature)
  osu <- density * c$carbon[9] * od(time, temperature)
  
  dau <- density * c$carbon[10] * dd(time, temperature)
  hau <- density * c$carbon[11] * hd(time, temperature)
  oau <- density * c$carbon[12] * od(time, temperature)
  
  dwi <- density * c$carbon[13] * dd(time, temperature)
  hwi <- density * c$carbon[14] * hd(time, temperature)
  owi <- density * c$carbon[15] * od(time, temperature)
...
CA <- function (species, period, time, slope) {
  ...
  d <- sum(c$density[1] * c$biomass[1] * dd0(time, mtdiff) * # exported biomass left after n days
           dh * dgpp(time) * PAR(time, slope, "year")) # GPP at n days
         
  h <- sum(c$density[2] * c$biomass[2] * hd0(time, mtdiff) *
           dh * hgpp(time) * PAR(time, slope, "year"))
  o <- sum(c$density[3] * c$biomass[3] * od0(time, mtdiff) *
           dh * ogpp(time) * PAR(time, slope, "year"))

  dsp <- sum(c$density[4] * c$biomass[4] * dd0(time, sptdiff) *
             spdh * dgpp(time) * PAR(time, slope, "spring"))
  hsp <- sum(c$density[5] * c$biomass[5] * hd0(time, sptdiff) *
             spdh * hgpp(time) * PAR(time, slope, "spring"))
  osp <- sum(c$density[6] * c$biomass[6] * od0(time, sptdiff) *
             spdh * ogpp(time) * PAR(time, slope, "spring"))

  dsu <- sum(c$density[7] * c$biomass[7] * dd0(time, sutdiff) *
             sudh * dgpp(time) * PAR(time, slope, "summer"))
  hsu <- sum(c$density[8] * c$biomass[8] * hd0(time, sutdiff) *
             sudh * hgpp(time) * PAR(time, slope, "summer"))
  osu <- sum(c$density[9] * c$biomass[9] * od0(time, sutdiff) *
             sudh * ogpp(time) * PAR(time, slope, "summer"))

  dau <- sum(c$density[10] * c$biomass[10] * dd0(time, autdiff) *
             audh * dgpp(time) * PAR(time, slope, "autumn"))
  hau <- sum(c$density[11] * c$biomass[11] * hd0(time, sptdiff) *
             audh * hgpp(time) * PAR(time, slope, "autumn"))
  oau <- sum(c$density[12] * c$biomass[12] * od0(time, sptdiff) *
             audh * ogpp(time) * PAR(time, slope, "autumn"))

  dwi <- sum(c$density[13] * c$biomass[13] * dd0(time, witdiff) *
             widh * dgpp(time) * PAR(time, slope, "winter"))
  hwi <- sum(c$density[14] * c$biomass[14] * hd0(time, sptdiff) *
             widh * hgpp(time) * PAR(time, slope, "winter"))
  owi <- sum(c$density[15] * c$biomass[15] * od0(time, sptdiff) *
...
#### 3.1   Present CSP ####
#### 3.1.1 Species ####
dCSP <- CSP(c$density[1], "digitata", "year", 0:250, mtdiff)
hCSP <- CSP(c$density[2], "hyperborea", "year", 0:250, mtdiff)
oCSP <- CSP(c$density[3], "ochroleuca", "year", 0:250, mtdiff)
...
c$density[1] * c$carbon[1] - c$density[1] * c$carbon[1] * dd(1, mtdiff) # slope = 0.8298488
# y = -0.83x + 89.85

# L. hyperborea
c$density[2] * c$carbon[2] # y-intercept = 211.1804
c$density[2] * c$carbon[2] - c$density[2] * c$carbon[2] * hd(1, mtdiff) # slope = 1.266776
# y = -1.27x + 211.18

# L. ochroleuca
c$density[3] * c$carbon[3] # y-intercept = 127.2262
c$density[3] * c$carbon[3] - c$density[3] * c$carbon[3] * od(1, mtdiff) # slope = 1.953599
# y = -1.95x + 127.23
...
# L. digitata
with(pseq, xint(time[1], time[2], CSP[1], CSP[2])) # mean = 108.2685 days
with(pseq, xint(time[1], time[2], hi[1], hi[2])) # upper = 132.8293 days
with(pseq, xint(time[1], time[2], lo[1], lo[2])) # lower = 83.70775 days

# L. hyperborea
with(pseq, xint(time[252], time[253], CSP[252], CSP[253])) # mean = 166.707 days
with(pseq, xint(time[252], time[253], hi[252], hi[253])) # upper = 209.0595 days
with(pseq, xint(time[252], time[253], lo[252], lo[253])) # lower = 124.3545 days

# L. ochroleuca
with(pseq, xint(time[503], time[504], CSP[503], CSP[504])) # mean = 65.12403 days
with(pseq, xint(time[503], time[504], hi[503], hi[504])) # upper = 86.78661 days
with(pseq, xint(time[503], time[504], lo[503], lo[504])) # lower = 43.46145 days

#### 3.1.2 Seasons ####
spdCSP <- CSP(c$density[4], "digitata", "spring", 0:400, sptdiff)
sphCSP <- CSP(c$density[5], "hyperborea", "spring", 0:400, sptdiff)
spoCSP <- CSP(c$density[6], "ochroleuca", "spring", 0:400, sptdiff)
sudCSP <- CSP(c$density[7], "digitata", "summer", 0:400, sutdiff)
suhCSP <- CSP(c$density[8], "hyperborea", "summer", 0:400, sutdiff)
suoCSP <- CSP(c$density[9], "ochroleuca", "summer", 0:400, sutdiff)
audCSP <- CSP(c$density[10], "digitata", "autumn", 0:400, autdiff)
auhCSP <- CSP(c$density[11], "hyperborea", "autumn", 0:400, autdiff)
auoCSP <- CSP(c$density[12], "ochroleuca", "autumn", 0:400, autdiff)
widCSP <- CSP(c$density[13], "digitata", "winter", 0:400, witdiff)
wihCSP <- CSP(c$density[14], "hyperborea", "winter", 0:400, witdiff)
wioCSP <- CSP(c$density[15], "ochroleuca", "winter", 0:400, witdiff)
...
# calculate temperature differentials to adjust decomposition rate
sst.fit.85$mtdiff <- sst.fit.85$mean.temp - fieldtemp
sst.fit.26$mtdiff <- sst.fit.26$mean.temp - fieldtemp
sst.fit.60$mtdiff <- sst.fit.60$mean.temp - fieldtemp

#### 3.2.5 Species ####
## RCP8.5
# 50 days is chosen because detritus of all species is still present at that stage
sst.fit.85$dCSP <- with(sst.fit.85, CSP(dp, "digitata", "year", 50, mtdiff))
sst.fit.85$hCSP <- with(sst.fit.85, CSP(hp, "hyperborea", "year", 50, mtdiff))
sst.fit.85$oCSP <- with(sst.fit.85, CSP(op, "ochroleuca", "year", 50, mtdiff))
...
## RCP2.6
# 50 days is chosen because detritus of all species is still present at that stage
sst.fit.26$dCSP <- with(sst.fit.26, CSP(dp, "digitata", "year", 50, mtdiff))
sst.fit.26$hCSP <- with(sst.fit.26, CSP(hp, "hyperborea", "year", 50, mtdiff))
sst.fit.26$oCSP <- with(sst.fit.26, CSP(op, "ochroleuca", "year", 50, mtdiff))
...
## RCP6.0
# 50 days is chosen because detritus of all species is still present at that stage
sst.fit.60$dCSP <- with(sst.fit.60, CSP(dp, "digitata", "year", 50, mtdiff))
sst.fit.60$hCSP <- with(sst.fit.60, CSP(hp, "hyperborea", "year", 50, mtdiff))
sst.fit.60$oCSP <- with(sst.fit.60, CSP(op, "ochroleuca", "year", 50, mtdiff))
...
# calculate species-specific CSP, ignoring the change in kelp forest composition
sst.fit.85$dCSP2 <- CSP(c$density[1], "digitata", "year", 50, sst.fit.85$mtdiff)
sst.fit.85$hCSP2 <- CSP(c$density[2], "hyperborea", "year", 50, sst.fit.85$mtdiff)
sst.fit.85$oCSP2 <- CSP(c$density[3], "ochroleuca", "year", 50, sst.fit.85$mtdiff)

sst.fit.85$CSP <- with(sst.fit.85, dCSP + hCSP + oCSP)
sst.fit.85$CSP2 <- with(sst.fit.85, dCSP2 + hCSP2 + oCSP2)

# build dataframe
fseqt.85 <- with(sst.fit.85, 
                 data.frame(time = 1900:2100,
                            CSP = c(CSP, CSP2),
                            group = c(rep("changed", 201), 
                                      rep("unchanged", 201))))
...
with(fseqt.85, (CSP[1] - CSP[201])/200) # decrease of 1.117664 (g C m-2 yr-1) per year
with(fseqt.85, (CSP[1] - CSP[201])/200/CSP[1]*100) # decrease of 0.37922% yr-1
with(fseqt.85, (CSP[1] - CSP[117])/116/CSP[1]*100) # decrease of 0.1939583% yr-1
with(fseqt.85, (CSP[117] - CSP[201])/84/CSP[117]*100) # decrease of 0.8194203% yr-1
...
# calculate species-specific CSP, ignoring the change in kelp forest composition
sst.fit.26$dCSP2 <- CSP(c$density[1], "digitata", "year", 50, sst.fit.26$mtdiff)
sst.fit.26$hCSP2 <- CSP(c$density[2], "hyperborea", "year", 50, sst.fit.26$mtdiff)
sst.fit.26$oCSP2 <- CSP(c$density[3], "ochroleuca", "year", 50, sst.fit.26$mtdiff)

sst.fit.26$CSP <- with(sst.fit.26, dCSP + hCSP + oCSP)
sst.fit.26$CSP2 <- with(sst.fit.26, dCSP2 + hCSP2 + oCSP2)
...
# estimated linear rate
with(fseqt.26, (CSP[1] - CSP[201])/200) # decrease of 0.6823112 (g C m-2 yr-1) per year
with(fseqt.26, (CSP[1] - CSP[201])/200/CSP[1]*100) # decrease of 0.2386945% yr-1
...
# calculate species-specific CSP, ignoring the change in kelp forest composition
sst.fit.60$dCSP2 <- CSP(c$density[1], "digitata", "year", 50, sst.fit.60$mtdiff)
sst.fit.60$hCSP2 <- CSP(c$density[2], "hyperborea", "year", 50, sst.fit.60$mtdiff)
sst.fit.60$oCSP2 <- CSP(c$density[3], "ochroleuca", "year", 50, sst.fit.60$mtdiff)

sst.fit.60$CSP <- with(sst.fit.60, dCSP + hCSP + oCSP)
sst.fit.60$CSP2 <- with(sst.fit.60, dCSP2 + hCSP2 + oCSP2)
...
# estimated linear rate
with(fseqt.60, (CSP[1] - CSP[201])/200) # decrease of 0.8071253 (g C m-2 yr-1) per year
with(fseqt.60, (CSP[1] - CSP[201])/200/CSP[1]*100) # decrease of 0.2809719% yr-1
...
pcs <- ggplot(pseq, aes(time, CSP)) +
          ...
          scale_colour_manual(values = c("#333b08","#627d0e","#f1c700"),
                              labels = c(expression(italic("L. digitata")*"        y = –0.83x + 89.85"),
                                         ...
                                         expression(italic("L. ochroleuca")*"  y = –1.95x + 127.23")),
                              guide = guide_legend()) +
...
pcss <- ggplot(pseqs, aes(time, CSP)) +
          ...
          scale_x_continuous(expand = c(0,0), breaks = seq(0, 400, by = 200)) +
          scale_y_continuous(expand = c(0,0)) +
          coord_cartesian(ylim = c(0, 200), xlim = c(0, 400)) +
          facet_grid(~season) +
...
fseqt.alt <- data.frame(rbind(fseqt.26, fseqt.60),
                        scenario = c(rep("RCP2.6", 402),
                                     rep("RCP6.0", 402)),
                        row.names = NULL)
fseqt.alt <- data.frame(fseqt.alt[-c(202:317, 604:719),], row.names = NULL)
fseqt.alt$lo[c(202:286, 488:572)] <- NA
fseqt.alt$hi[c(202:286, 488:572)] <- NA
fseqt.alt$group <- factor(fseqt.alt$group, levels = c("unchanged", "changed"))

fcst2 <- ggplot(fseqt.alt, aes(time, CSP)) +
          annotate("segment", x = Inf, y = Inf, xend = -Inf, yend = Inf, lwd = 1) +
          geom_line(aes(colour = group), size = 0.5) +
          geom_ribbon(aes(fill = group, ymin = lo, ymax = hi), alpha = 0.5) +
          geom_vline(xintercept = 1946) +
          geom_vline(data = fseqt.alt[fseqt.alt$scenario == "RCP2.6",], aes(xintercept = 2095)) +
          scale_colour_manual(values = c("#be3819", "#f5a54a"),
                              labels = c("Unchanged composition",
                                         "Changed composition")) +
          scale_fill_manual(values = c("#be3819", "#f5a54a"),
                            labels = c("Unchanged composition",
                                       "Changed composition")) +
          ylab(expression("Carbon sequestration (g C m"^-2*" yr"^-1*")")) +
          xlab("Year") +
...
## RCP8.5
fseqt <- data.frame(fseqt.85[c(1:201, 318:402),], row.names = NULL)
fseqt$lo[202:286] <- NA
fseqt$hi[202:286] <- NA
fseqt$group <- factor(fseqt$group, levels = c("unchanged", "changed"))

fcst <- ggplot(fseqt, aes(time, CSP)) +
          geom_line(aes(colour = group), size = 0.5) +
          geom_ribbon(aes(fill = group, ymin = lo, ymax = hi), alpha = 0.5) +
          geom_vline(xintercept = 1946) +
          geom_vline(xintercept = 2090) +
          geom_rug(fseqt.alt[c(201, 487),], mapping = aes(CSP), colour = "#f5a54a") +
          geom_text(fseqt.alt[c(201, 487),], 
                    mapping = aes(1925, CSP, label = scenario), size = 4.2) +
          scale_colour_manual(values = c("#be3819", "#f5a54a"),
                              labels = c("Unchanged composition",
                                         "Changed composition")) +
          scale_fill_manual(values = c("#be3819", "#f5a54a"),
...
seasonal <- plot_grid(pcss, pcas, labels = "auto", ncol = 1,
                      label_size = 15)
seasonal # dimensions 8 x 8.5 in

fcst <- fcst + theme(axis.title.y = element_blank(),
                     axis.text.y = element_blank(),
                     plot.margin = unit(c(.2, .5, .2, .5),"cm"))
main <- plot_grid(pcs, fcst, labels = "auto", 
                  rel_widths = c(1.12, 1), hjust = 0.5, label_size = 15)
main # dimensions 4 x 8.5 in
